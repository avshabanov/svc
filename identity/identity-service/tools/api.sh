...
# Default parameters
DEFAULT_PROPS="
brikar.tools.host=http://127.0.0.1
brikar.tools.port=8080
brikar.tools.username=testonly
brikar.tools.password=test
"

function prop {
  echo "$DEFAULT_PROPS" | grep "${1}" | cut -d'=' -f2
}
...
case $ACTION in
  ...
  list-accounts)
    ...
    invokeApi "POST" "rpc/IdentityServiceV1/listAccounts" "{\"limit\": $LIMIT $OFFSET_TOKEN_KV}"
    ;;

  lookup-account)
    USERNAME=$2
    invokeApi "POST" "rpc/IdentityServiceV1/lookupAccount" "{\"username\": \"$USERNAME\"}"
    ;;
  ...
  create-invitation-token)
    ...
    if [ -z "$EXPIRATION_TIME" ]; then
      # TODO: doesn't work in Mac, support relative time
      EXPIRATION_TIME=$(($(date +%s%N)/1000000+3600000))
    fi
    invokeApi "POST" "rpc/IdentityServiceV1/createInvitationToken" "{\"authorities\": $AUTHORITIES, \"count\": 1, \"expirationTime\": $EXPIRATION_TIME}"
    ;;
  ...
  register-account)
    ...
    invokeApi "POST" "rpc/IdentityServiceV1/registerAccount" "$REGISTRATION_REQUEST"
    ;;
  ...
  update-account)
    ...
    invokeApi "PUT" "rpc/IdentityServiceV1/updateAccount" "$UPDATE_REQUEST"
    ;;
